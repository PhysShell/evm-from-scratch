...
class ReconcileTest(unittest.TestCase):
    ...
    def test_footer_carrying_trigger_body_binds(self):
        """Real transport shape from PR #11: command plus attribution footer
        (comment 5364757271). First-line matching must bind it."""
        body = "@codex review\n\n---\n_Generated by [Claude Code](https://claude.ai/code)_"
        updated = reconcile_unknown_request(
            self.store,
            self.run,
            GOVERNOR_ACTOR_ID,
            [self._own_comment(1, 616161, body=body)],
            listing_complete=True,
            now=ts(2),
        )
        self.assertEqual(updated.request_state, RequestState.REQUEST_BOUND)
        self.assertEqual(updated.request_comment_id, 616161)
